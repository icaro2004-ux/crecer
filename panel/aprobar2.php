...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'arte') {
        @set_time_limit(0);
        // TOPES DUROS — aplican SIEMPRE (incluidos admins y cuentas de prueba):
        // máx 2 imágenes por post y 10 por semana (= 5 posts). No matter what.
        // (En la revisión decidiremos si los subimos.)
...
if ($es_hub) {
    ...
    // FIX (entrada directa a CREAR): el redirect del hub botaba ?crear=1&idea=…
    // y el wizard nunca abría solo — preservarlos.
    if (($_GET['crear'] ?? '') === '1') {
        $extra .= '&crear=1';
...
<script>
  ...
  // ===== WIZARD de crear: extraído a _crear_wizard.php (compartido con El Estudio) =====
...
<?php $wiz_host_con_loader = true; include __DIR__ . '/_crear_wizard.php'; ?>
